...
class KeplerMapper(object):
    ...
    def map(self, X, dimension_index=[0], dimension_name=""):
        ...
        for i, coor in enumerate(cube_coordinates_all(self.nr_cubes, di.shape[0])):
            ...
            hypercube = X[np.invert(np.any((X[:, di + 1] >= self.d[di] + (coor * self.chunk_dist[di])) &
                                           (X[:, di + 1] < self.d[di] + (coor * self.chunk_dist[di]) + self.chunk_dist[
                                               di] + self.overlap_dist[di]) == False, axis=1))]
                ...
                for a in np.c_[hypercube[:, 0], clf.labels_]:
                    if a[1] != -1:  # if not predicted as noise
                        cluster_id = str(coor[0]) + "_" + str(i) + "_" + str(a[1]) + "_" + str(coor) + "_" + str(
                            self.d[di] + (coor * self.chunk_dist[di]))  # Rudimentary cluster id
            ...
            for kn in nodes:
                if k != kn:
                    if len(nodes[k] + nodes[kn]) != len(
                            set(nodes[kn] + nodes[k])):  # there are non-unique id's in the union
                        links[k].append(kn)
    ...
    def visualize(self, complex, path_html="mapper_visualization_output.html", title="My Data", graph_link_distance=30,
                  graph_gravity=0.1, graph_charge=-120, custom_tooltips=None):
		...
		* {margin: 0; padding: 0;}
		...
		var divs = d3.select('#holder').append('div')
            ...
            title, title, complex["meta"], self.nr_cubes, self.overlap_perc * 100, self.link_local, self.color_function,
            complex["meta"], str(self.clf), str(self.scaler), graph_charge, graph_link_distance, graph_gravity,
            json.dumps(json_s))
...
if __name__ == "__main__":
    ...
    mapper.visualize(complex, "results.html", "t-SNE on first 4k images", graph_charge=-50, graph_gravity=0.15,
